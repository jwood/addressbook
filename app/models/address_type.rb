class AddressType < ActiveRecord::Base
  ...
  def format_address_for_label(address)
    if get_type == :family
      ...
      "#{address.primary_contact.prefix} & #{address.secondary_contact.try(:prefix)} #{address.primary_contact.first_name} & #{address.secondary_contact.try(:first_name)} #{address.primary_contact.last_name}"
    elsif get_type == :unmarried_couple
      "#{address.primary_contact.prefix} #{address.primary_contact.first_name} #{address.primary_contact.last_name} & #{address.secondary_contact.try(:prefix)} #{address.secondary_contact.try(:first_name)} #{address.secondary_contact.try(:last_name)}"
    elsif get_type == :single_parent
      "The #{address.primary_contact.last_name} Family"
    end
  end
