...
class GroupControllerTest < Test::Unit::TestCase
  ...
  def test_edit_group_post
    ...
    group.name = 'New Name'
    post :edit_group, { :id => group.id, :group => group.attributes }
    assert_template 'edit_group'
  ...
  def test_add_address_to_group
    ...
    post :add_address_to_group, { :id => address.id, :group_id => group.id }
  ...
  def test_remove_address_from_group
    ...
    post :remove_address_from_group, { :id => address.id, :group_id => group.id }
    assert_template 'update_address_group_lists'
    assert(!assigns(:included).include?(address))
  ...
  def test_add_all_addresses_to_group
    group = groups(:group_1)
    post :add_all_addresses, { :id => group.id }
  ...
  def test_remove_all_address_from_group
    ...
    post :remove_all_addresses, { :id => group.id }
  ...
  def test_finish_maintaining_group
    group = groups(:group_1)
    get :finish_maintaining_group, { :id => group.id }
