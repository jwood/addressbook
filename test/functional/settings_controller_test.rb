...
class SettingsControllerTest < ActionController::TestCase
  ...
  context "on POST to :address with a valid address" do
    setup do
      assert Settings.save_home_address(Address.new(:address1 => "123 South Ave", :city => "Chicago", :state => "IL", :zip => "60606"))
  ...
  context "on POST to :address with an invalid address" do
    setup do
      assert Settings.save_home_address(Address.new(:address1 => "123 South Ave", :city => "Chicago", :state => "IL", :zip => "60606"))
  ...
  context "on GET to :login_credentials" do
    setup do
  ...
  context "on POST to :login_credentials with a missing password and password confirmation" do
    setup { xhr :post, :login_credentials, :username => 'bob' }
  ...
  context "on POST to :login_credentials with a non matching password and password confirmation" do
    setup { xhr :post, :login_credentials, :username => 'bob', :password => 'mypass', :password_confirmation => 'notmypass' }
  ...
  context "on POST to :login_credentials with a valid username, password, and password confirmation" do
    setup { xhr :post, :login_credentials, :username => 'bob', :password => 'mypass', :password_confirmation => 'mypass' }
  ...
  context "on POST to :login_credentials when existing login credentials are already set" do
    setup do
      Settings.username = 'bobby'
      Settings.password = 'pass'
