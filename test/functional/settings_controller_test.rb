...
class SettingsControllerTest < ActionController::TestCase
  ...
  context "on PUT to :update_address with a valid address" do
    setup do
      assert Settings.save_home_address(Address.new(:address1 => "123 South Ave", :city => "Chicago", :state => "IL", :zip => "60606"))
  ...
  context "on PUT to :update_address with an invalid address" do
    setup do
      assert Settings.save_home_address(Address.new(:address1 => "123 South Ave", :city => "Chicago", :state => "IL", :zip => "60606"))
  ...
  context "on PUT to :update_login_credentials with a missing password and password confirmation" do
    setup { xhr :put, :update_login_credentials, :username => 'bob' }
  ...
  context "on PUT to :update_login_credentials with a non matching password and password confirmation" do
    setup { xhr :put, :update_login_credentials, :username => 'bob', :password => 'mypass', :password_confirmation => 'notmypass' }
  ...
  context "on PUT to :update_login_credentials with a valid username, password, and password confirmation" do
    setup { xhr :put, :update_login_credentials, :username => 'bob', :password => 'mypass', :password_confirmation => 'mypass' }
  ...
  context "on PUT to :update_login_credentials when existing login credentials are already set" do
    setup do
      Settings.username = 'bobby'
      Settings.password = 'pass'
  ...
  context "on PUT to :update_login_credentials when trying to change existing credentials but specified the wrong current password" do
    setup do
      Settings.username = 'bobby'
      Settings.password = 'pass'
  ...
  context "on PUT to :update_login_credentials when removing the login credentials" do
    setup do
