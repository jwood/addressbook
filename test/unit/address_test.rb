...
class AddressTest < ActiveSupport::TestCase
  ...
  context "An Address" do
    should "not be created without all required info" do
      address = Address.new
      assert !address.valid?
      assert address.errors[:base].include?('You must specify a phone number or a full address')
    end

    should "list the phone number as the addressee if there is no street address and no contacts" do
      address = addresses(:phone_only)
      assert_equal('708-111-3333', address.addressee)
    end

    should "list the phone number as the addressee for display if there is only a phone number" do
      address = addresses(:phone_only)
      assert_equal('708-111-3333', address.addressee_for_display)
    end

    should "be considered equal when sorted when they have no primary contacts" do
      chicago = addresses(:chicago)
      alsip = addresses(:alsip)
      assert_equal(0, chicago.compare_by_primary_contact(alsip))
    end

    should "appear before addresses without primary contacts when sorted if they have a primary contact" do
      chicago = addresses(:chicago)
      alsip = addresses(:alsip)
    ...
    should "be sorted by the name of their primary contact if both addresses have primary contacts" do
      chicago = addresses(:chicago)
      chicago.primary_contact = contacts(:john_doe)
    ...
    should "be considered empty if it has no information or only a home phone number" do
    ...
    should "not be considered empty if it has no street address but a valid home phone" do
      assert Address.new.is_empty?
      assert !Address.new(:home_phone => '312-555-1212').is_empty?
      assert !addresses(:alsip).is_empty?
    end

    should "be able to get the mailing address for an address" do
    ...
    should "be able to determine if one address is different from another" do
    ...
    should "be able to easily tell if an address only has one contact" do
    ...
    should "not be able to edit the address type of an address without the required primary and secondary contacts" do
      assign_contact_to_address(contacts(:john_doe), addresses(:chicago))
    ...
    should "fail validation with an invalid state code" do
      address = addresses(:chicago)
      address.state = "XY"
      assert !address.valid?
    end

    should "fail validation with an invalid zip code" do
      address = addresses(:chicago)
      address.zip = "abcd"
      assert !address.valid?
    end

    should "not consider partial addresses valid" do
      address = addresses(:chicago)
      address.city = ""
      assert !address.valid?
    end

    should "be able to delete an address that is a member of a group" do
      address = addresses(:alsip)
    ...
    should "not be able to update an address with an invalid phone number" do
    ...
    should "simply list the street address if it has no contacts" do
      assert_equal('123 Main St. Apt. 109, Chicago, IL 60606', addresses(:chicago).addressee)
    end

    should "list the contact's name as the addressee for a single contact" do
    ...
    should "list both contacts names as the addressee for a married couple" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
    ...
    should "list the family name as the addressee for a family" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).secondary_contact = contacts(:jane_doe)
    ...
    should "list both contacts first and last names as the addressee if for an unmarried couple" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).secondary_contact = contacts(:jane_doe)
    ...
    should "list the familiy name as the adressee if for a single parent" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).address_type = address_types(:single_parent)
      assert_equal('The Doe Family', addresses(:chicago).addressee)
    end

    should "list the street address as the addressee for display if it has no contacts" do
      assert_equal('123 Main St. Apt. 109, Chicago, IL 60606', addresses(:chicago).addressee_for_display)
    end

    should "list that contact's name as the addressee for display for an individual" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).address_type = address_types(:individual)
      assert_equal('Doe, Mr. John', addresses(:chicago).addressee_for_display)
    end

    should "list both contacts as the addressee for display for a married couple" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).secondary_contact = contacts(:jane_doe)
      addresses(:chicago).address_type = address_types(:married_couple)
    ...
    should "list the primary contacts and family name as the addressee for display for a family" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).secondary_contact = contacts(:jane_doe)
      addresses(:chicago).address_type = address_types(:family)
    ...
    should "list both contacts first and last names as the addressee for display for an unmarried couple" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).secondary_contact = contacts(:jane_doe)
      addresses(:chicago).address_type = address_types(:unmarried_couple)
    ...
    should "list the contact name and the family name in the addressee for display for a single parent" do
      addresses(:chicago).primary_contact = contacts(:john_doe)
      addresses(:chicago).address_type = address_types(:single_parent)
      assert_equal('Doe, Mr. John & Family', addresses(:chicago).addressee_for_display)
    end

    should "not be able to compare an address with something that is not an Address" do
      assert_raise ArgumentError do
        addresses(:chicago).compare_by_primary_contact("not an address")
      end
    end

    should "be able to find all addresses to list in the app" do
      chicago = addresses(:chicago)
      chicago.primary_contact = contacts(:john_doe)
      ...
      assert_same_elements(Address.all, addresses)
    ...
    should "be able to find all addresses eligible to be a member in a group" do
    ...
    should "be able to unlink a contact from an address and delete the address if it only contains one contact" do
    ...
    context "with linked contacts" do
      setup do
        @john = contacts(:john_doe)
      ...
      should "be able to unlink the secondary contact from an address, adjusting the main contacts accordingly" do
        # Jane moves out
      ...
      should "be able to unlink the primary contact from an address, adjusting the main contacts accordingly" do
        # John moves out
        @john.address_id = nil; @john.save
        addresses(:chicago).unlink_contact(@john)
      ...
      should "be able to unlink the secondary and primary contacts from an address, adjusting the main contacts accordingly" do
        # John moves out
        @john.address_id = nil; @john.save
        addresses(:chicago).unlink_contact(@john)
      ...
      should "nullify the contact's address id if an address is deleted" do
